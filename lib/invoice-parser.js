...
function parseInvoiceEmail({ from = '', subject = '', text = '' }, today = new Date()) {
  ...
  return {
    ...
    // Enough of the mail that a person can settle a question by reading it. 400 characters cut off
    // mid-sentence on longer invoices, and sliced a forwarding confirmation link clean in half —
    // the kind of limit you only discover at the moment you need what was cut.
    snippet: hay.slice(0, 2000),
  };
}
